package gui;
import java.awt.BorderLayout;
import java.awt.Button;

import javax.swing.JButton;
import javax.swing.JFrame;
import javax.swing.JPanel;

import listener.addbutton;
import listener.viewbutton;

import javax.swing.JLabel;

public class menuselection extends JPanel{
	
	WindowFrame frame;
	
	
	public menuselection(WindowFrame frame) {
		
		this.frame= frame;
		
		this.setLayout(new BorderLayout());
		
		JPanel panel1=new JPanel();
		JPanel panel2=new JPanel();
		JLabel label = new JLabel ("Menu Selection");
		JButton button1 = new JButton("Add student ");
		JButton button2 = new JButton("View student ");
		JButton button3 = new JButton("Edit student ");
		JButton button4 = new JButton("Delete student ");
		JButton button5 = new JButton("Exit student ");
		
		button1.addActionListener(new addbutton(frame)); 
		button2.addActionListener(new viewbutton(frame)); 
		
		panel1.add(label);
		panel2.add(button1);
		panel2.add(button2);
		panel2.add(button3);
		panel2.add(button4);
		panel2.add(button5);
		
		this.add(panel1, BorderLayout.NORTH);
		this.add(panel2,BorderLayout.CENTER);
		
	
		
	
	
	}

}
